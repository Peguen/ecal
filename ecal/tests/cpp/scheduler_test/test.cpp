...
public:
    ...
    size_t SendOutputs() {       
        ...
        for(auto& pub : m_output_publisher) {
            std::string message = "I send something to you!" + std::to_string(send_counter++);
            all_send_bytes += pub.Send(message.data(), message.size());        
        }
